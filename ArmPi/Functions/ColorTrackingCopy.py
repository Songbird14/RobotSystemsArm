...
#Set the RGB light color of the explansion board to match the color you want to track
def set_rgb(color):
    if color == "red":
...
def move():
    ...
    # Coordinates (x, y, z) of the different colored blocks
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # When an object is first detected               
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) #Do not fill in the running time parameter
                                                                                        #the running time will be adaptive
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if start_pick_up: #If the object has not moved for a while start gripping
                    ...
                    initMove()  # Return to intial position
                    time.sleep(1.5)
